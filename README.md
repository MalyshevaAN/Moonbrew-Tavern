...
8. После завершения очереди начинается новая ночь.
...
| **FR-06** | В зале должен работать таймер ночи, который приостанавливается на других игровых экранах и продолжается после возврата. | Реализовано |
...
| **FR-20** | После завершения очереди система должна увеличить номер ночи и сформировать новую очередь. | Реализовано |
...
| **FR-22** | Прогресс должен сохраняться после полного закрытия и повторного запуска приложения. | Запланировано |
...
```text
Очередь → Зал → Диалог → Книга рецептов → Варка → Зал → Результат
```

Таймер работает в зале и ставится на паузу на экранах диалога, книги рецептов, варки и результата.

## Хранение прогресса

На текущем этапе состояние хранится в памяти процесса приложения. В течение игровой сессии сохраняются:
...
Постоянное сохранение после полного закрытия приложения пока не реализовано. В дальнейшем репозиторий можно подключить к Room или DataStore без изменения основной структуры UI.
...
В прототип пока не входят:

- постоянное сохранение между запусками;
- музыка и звуковые эффекты;
- полноценная система улучшений таверны;
- случайные события;
